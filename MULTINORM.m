...
for i=1:lt
    for t=2:N+1
        ...
        %h=exp(logratio)./((ones(n,1)+exp(logratio)).^2);
        %fisher=X_ext'*diag(h)*X_ext+diag(0.1^-2,0.1^-2);
        beta(t,lt*(i-1)+1:lt*(i-1)+2)=mvnrnd(beta(t-1,lt*(i-1)+1:lt*(i-1)+2),(cbeta^2)*inv(fisher(beta(t-1,lt*(i-1)+1:lt*(i-1)+2),X_ext)));
        u=rand;
        beta_prior=beta(t-1,lt*(i-1)+1:lt*(i-1)+2);
        beta_dot=beta(t,lt*(i-1)+1:lt*(i-1)+2);
        nume=evaluate_loglikelihood(X, y, beta_dot')+log(normpdf(beta_dot(1),0,100)*normpdf(beta_dot(2),0,100))+logmvnpdf(beta_prior,beta_dot,cbeta^2*inv(fisher(beta_dot,X_ext)));
        deno=evaluate_loglikelihood(X, y, beta_prior')+log(normpdf(beta_prior(1),0,100)*normpdf(beta_prior(2),0,100))+logmvnpdf(beta_dot,beta_prior,cbeta^2*inv(fisher(beta_prior,X_ext)));
...
%%
%Fisher信息阵
function [fishermatrix]=fisher(be,xmat)
logratio = xmat * be';
n=size(xmat,1);
h=exp(logratio)./((ones(n,1)+exp(logratio)).^2);
fishermatrix=xmat'*diag(h)*xmat+diag([0.1^-2,0.1^-2]);
end